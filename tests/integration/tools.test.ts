...
describe.skipIf(!HAS_DSS)("Integration: MCP tools against live DSS", () => {
  ...
  describe("project", () => {
    ...
    it("map — gets normalized flow map with connectivity", async () => {
      const { text, structured, isError } = await call("project", { action: "map" });
      expect(isError).toBeFalsy();
      expect(text).toContain("Flow map for");
      const data = (structured?.map ?? {}) as {
        nodes?: Array<{ kind?: string; subtype?: string }>;
        edges?: unknown[];
      ...
      const recipeNode = data.nodes?.find((node: { kind?: string }) => node.kind === "recipe");
      if (recipeNode) {
    ...
    it("map — includes raw flow payload when includeRaw=true", async () => {
      const { text, structured, isError } = await call("project", {
        action: "map",
        includeRaw: true,
      });
      expect(isError).toBeFalsy();
  ...
  describe("job", () => {
    ...
    it("log — gets job log", async () => {
      ...
      const { isError: waitError, structured: waitStructured } = await call("job", {
        action: "wait",
        jobId,
        timeoutMs: 30_000,
      ...
      if (waitError) {
        expect(waitStructured?.normalizedState).toMatch(/terminalFailure|timeout/);
      }
    ...
    it("wait — returns terminal status for an existing terminal job", async () => {
      const { text: listText, isError: listError } = await call("job", {
        action: "list",
      });
      expect(listError).toBeFalsy();
      ...
      const terminalJob = listText
        .split("\n")
        .map((line) => line.match(/^•\s+(\S+)\s+\[(\S+)\]/))
        .find((match) => terminalStates.has((match?.[2] ?? "").toUpperCase()));
      if (!terminalJob?.[1] || !terminalJob[2]) return;

      const expectedNormalizedState =
        terminalJob[2].toUpperCase() === "DONE" ? "terminalSuccess" : "terminalFailure";

      const { text, isError, structured } = await call("job", {
        action: "wait",
        jobId: terminalJob[1],
        timeoutMs: 5_000,
        pollIntervalMs: 500,
      });
      expect(structured?.normalizedState).toBe(expectedNormalizedState);
      expect(Boolean(isError)).toBe(expectedNormalizedState === "terminalFailure");
      expect(text).toContain(`Job: ${terminalJob[1]}`);
  ...
  describe("error handling", () => {
    ...
    it("returns clean error for non-existent recipe", async () => {
      const { text, isError } = await call("recipe", {
        action: "get",
        recipeName: "DOES_NOT_EXIST_12345",
      });
