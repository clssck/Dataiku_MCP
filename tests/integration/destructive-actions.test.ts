...
describe.skipIf(!(HAS_DSS && RUN_DESTRUCTIVE))(
   ...
   () => {
      ...
      const TS = Date.now();
      const INPUT_DS = `_test_in_${TS}`;
      const OUTPUT_DS = `_test_out_${TS}`;
      const RECIPE_NAME = `python_${OUTPUT_DS}`;
      const SCENARIO_ID = `_test_run_${TS}`;
      const FOLDER_REMOTE_PATH = `_mcp_test/${TS}/smoke.txt`;
      ...
      async function cleanupBestEffort() {
         for (const recipeName of createdRecipes) {
            await call("recipe", { action: "delete", recipeName }).catch(() => null);
         }
         for (const datasetName of createdDatasets) {
            await call("dataset", { action: "delete", datasetName }).catch(() => null);
         }
         for (const scenarioId of createdScenarios) {
            await call("scenario", { action: "delete", scenarioId }).catch(() => null);
         }
         if (folderId) {
            await call("managed_folder", {
               action: "delete_file",
               folderId,
