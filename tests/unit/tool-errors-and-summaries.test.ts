...
describe("Tool Errors And Summaries", () => {
  ...
  it("job build errors when datasetName is missing", async () => {
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/datasetName|Input validation error/i);
  });
  ...
  it("job get errors when jobId is missing", async () => {
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/jobId|Input validation error/i);
  });

  it("job wait errors when jobId is missing", async () => {
    const { text, isError } = await callTool(registerJobs, "job", {
      action: "wait",
      projectKey: "PROJ",
    });
  ...
  it("managed_folder download errors when path is missing", async () => {
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/path|Input validation error/i);
  });
  ...
  it("managed_folder get errors when folderId is missing", async () => {
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/folderId|Input validation error/i);
  });
  ...
  it("code_env get errors when envName/envLang are missing", async () => {
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/envName|Input validation error/i);
  });

  it("code_env get rejects empty envName at schema level", async () => {
    const { text, isError } = await callTool(registerCodeEnvs, "code_env", {
      action: "get",
      envLang: "PYTHON",
      envName: "",
  ...
  it("code_env get rejects non-positive maxPackages at schema level", async () => {
    const { text, isError } = await callTool(registerCodeEnvs, "code_env", {
      action: "get",
      envLang: "PYTHON",
      envName: "py39",
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/maxPackages|greater than or equal to 1|Input validation error/i);
  });

  it("managed_folder contents rejects non-positive limit at schema level", async () => {
    const { text, isError } = await callTool(registerFolders, "managed_folder", {
      action: "contents",
      projectKey: "PROJ",
      folderId: "folder_1",
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/limit|greater than or equal to 1|Input validation error/i);
  });

  it("project list rejects non-positive limits at schema level", async () => {
  ...
  it("connection rejects unsupported actions at schema level", async () => {
    const { text, isError } = await callTool(registerConnections, "connection", {
      action: "list",
      projectKey: "PROJ",
    });
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/action|Input validation error/i);
  });

  it("variable set rejects non-object standard patches at schema level", async () => {
    const { text, isError } = await callTool(registerVariables, "variable", {
      action: "set",
      projectKey: "PROJ",
      standard: "not-object",
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/standard|Input validation error|object/i);
  });

  it("variable get rejects non-positive maxKeys at schema level", async () => {
    const { text, isError } = await callTool(registerVariables, "variable", {
      action: "get",
      projectKey: "PROJ",
      maxKeys: 0,
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/maxKeys|greater than or equal to 1|Input validation error/i);
  });
  ...
  it("dataset create errors when required fields are missing", async () => {
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/connection|Input validation error/i);
  });
  ...
  it("recipe get returns a clean error when DSS returns an empty definition envelope", async () => {
    clientMocks.get.mockResolvedValue({});

    const { text, isError } = await callTool(registerRecipes, "recipe", {
      action: "get",
      projectKey: "PROJ",
      recipeName: "MISSING_RECIPE",
    });

    expect(isError).toBe(true);
    expect(text).toContain("was not found or returned an empty definition");
  ...
  it("scenario run errors when scenarioId is missing", async () => {
    ...
    expect(isError).toBe(true);
    expect(text).toMatch(/scenarioId|Input validation error/i);
  });
