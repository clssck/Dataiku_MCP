...
export function normalizeFlowGraph(
	...
	for (const [nodeKey, nodeValue] of Object.entries(asRecord(rawNodes) ?? {})) {
		...
		const fallbackName =
			asString(nodeObj.name) ?? asString(nodeObj.label) ?? ref ?? nodeKey;
		const folderName =
			kind === "folder" ? asString(options.folderNamesById?.[id]) : undefined;
		...
		node.successors = asStringArray(
			nodeObj.successors,
			warnings,
			`nodes.${nodeKey}.successors`,
		);
	}
